...
(
 ...
 attribute vec4 mColorsArray;
 
 uniform float crosshairWidth;
 
 varying lowp vec2 centerLocation;
 varying lowp float pointSpacing;
 varying lowp vec4 colorsArray;
 
 void main()
 {
     
     gl_Position = vec4(position.xy, 0.0, 1.0); //vec4(((position.xy * 2.0) - 1.0), 0.0, 1.0);
     colorsArray = mColorsArray;
...
(
 ...
 void main()
 {
     ...
     gl_FragColor = vec4(colorsArray);
	//gl_FragColor = vec4(crosshairColor, 0.5);
...
- (id)init;
{
    ...
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        [self setCrosshairColorRed:1.0 green:1.0 blue:0.0];
    });
